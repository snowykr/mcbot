...
cd mcbot && go test ./internal/cli/... -run 'TestSetupConfigInteractiveFlow|TestSetupConfigUsesExistingValuesAsDefaults|TestSetupConfigRejectsInvalidTypedValues|TestSetupConfigOwnershipUsesExistingDataOwner|TestSetupConfigOwnershipStatErrorFails|TestSetupYesUsesDefaults'
...
cd mcbot && ./mcbot env get --file /tmp/test.env DISCORD_TOKEN
...
다음 항목은 운영 비밀값과 Discord 접근이 준비된 경우에만 확인합니다.

### /마크봇 채널 수동 QA

- [ ] 신뢰한 길드에서 `/마크봇 채널 설정 channel:<채널>` 실행 시 새 채널에 상시 임베드가 표시된다
- [ ] `/마크봇 채널 기본값` 실행 시 저장된 런타임 설정이 삭제되고 `.env` 기본 채널로 되돌아간다
- [ ] `/마크봇 채널 끄기` 실행 시 비활성화 상태가 저장되고, 재시작 후에도 `.env` fallback보다 우선해 상태 임베드가 비활성화된다
- [ ] 다른 길드에서 `/마크봇 채널 ...` 실행 시 거부된다
- [ ] `마크봇` 역할이 없는 계정은 채널 변경이 거부된다
- [ ] 잘못된 입력, 길드 검증 실패, 권한 실패는 에페멀 메시지로 거부된다
- [ ] 손상된 `./data/mcbot/runtime-config.json`이 있으면 경고 후 `.env` 기본값으로 안전하게 되돌아간다
- [ ] `MCBOT_TRUSTED_GUILD_ID`를 바꾸거나 guild scope가 없는 legacy `runtime-config.json`이 있으면 무시/삭제되고 `.env` 기본값으로 되돌아간다
- [ ] 오래된 채널이 남아 있어도 자동 삭제되지 않는다
- [ ] 채널 변경/끄기 뒤 재시작해도 런타임 설정이 유지되고 .env보다 우선한다
...
#### 2.3 채널 변경 후 이전 채널 정리 여부

**절차**:
1. `/마크봇 채널 설정 channel:<채널>`로 새 채널을 지정
2. 이전 채널의 메시지 여부 확인

**예상 결과**:
- 새 채널에 상시 임베드가 유지됨
- 이전 채널 메시지는 자동으로 삭제되지 않음
- 이전 채널을 직접 지워야 하면 수동으로 정리해야 함
