...
func renderComposeConfig(t *testing.T, overrides map[string]string) composeConfig {
	...
	envFilePath := composeConfigEmptyEnvFile(t)
...
func composeConfigEmptyEnvFile(t *testing.T) string {
	t.Helper()

	envFilePath := filepath.Join(t.TempDir(), "compose.env")
	if err := os.WriteFile(envFilePath, []byte{}, 0o600); err != nil {
		t.Fatalf("create empty compose env file failed: %v", err)
	}
